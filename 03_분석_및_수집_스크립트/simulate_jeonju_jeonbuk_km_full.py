...
전주시 및 전북특별자치도 14개 시·군 한의약 방문진료 및 돌봄케어 기관 참여율 10%~100% (10단계)
...
REGIONS = {
    'jeonju': {
        ...
        'name': '전주시',
        'pop_total': 651000,
        'pop_elderly': 122000,       # 18.7%
        'pop_vulnerable': 38000,     # 5.8%
        'pop_non_elderly': 529000,
        'base_eld_cost': 5400000.0,  # 540.0만 원
        'base_vul_cost': 7900000.0,  # 790.0만 원
        'base_non_cost': 1400000.0,  # 140.0만 원
        'km_hosp': 22,               # 한방병원 22개소
        'km_clinic': 285,            # 한의원 285개소
        'km_total_insts': 307,
        'km_doctors': 480            # 한의사 약 480명
    },
    'jeonbuk': {
        'id': 'jeonbuk',
        'name': '전북특별자치도 (전체 14개 시·군)',
        'pop_total': 1755000,
        'pop_elderly': 445000,       # 25.4% (초고령사회)
        'pop_vulnerable': 128000,    # 7.3%
        'pop_non_elderly': 1310000,
        'base_eld_cost': 5850000.0,  # 585.0만 원
        'base_vul_cost': 8350000.0,  # 835.0만 원
        'base_non_cost': 1450000.0,  # 145.0만 원
        'km_hosp': 38,               # 한방병원 38개소
        'km_clinic': 612,            # 한의원 612개소
        'km_total_insts': 650,
        'km_doctors': 1020           # 한의사 약 1,020명
    }
}
...
DISEASE_COMPARISON = [
    {
        ...
        'prevalence_eld': 0.42,
        'conv_annual_cost': 4850000.0,
        'km_annual_cost': 1650000.0,
        'saving_per_patient': 3200000.0,
        'saving_pct': 66.0,
    ...
    },
    {
        ...
        'prevalence_eld': 0.12,
        'conv_annual_cost': 14200000.0,
        'km_annual_cost': 3800000.0,
        'saving_per_patient': 10400000.0,
        'saving_pct': 73.2,
    ...
    },
    {
        ...
        'prevalence_eld': 0.14,
        'conv_annual_cost': 11800000.0,
        'km_annual_cost': 3100000.0,
        'saving_per_patient': 8700000.0,
        'saving_pct': 73.7,
    ...
    },
    {
        ...
        'prevalence_eld': 0.28,
        'conv_annual_cost': 5600000.0,
        'km_annual_cost': 1900000.0,
        'saving_per_patient': 3700000.0,
        'saving_pct': 66.1,
    ...
    },
    {
        ...
        'prevalence_eld': 0.06,
        'conv_annual_cost': 16500000.0,
        'km_annual_cost': 4500000.0,
        'saving_per_patient': 12000000.0,
        'saving_pct': 72.7,
...
# 전주시 및 전북특별자치도 시뮬레이션 실행
np.random.seed(42)
jeonju_results = run_simulation_for_region('jeonju')
jeonbuk_results = run_simulation_for_region('jeonbuk')

# JSON 저장
output_path = "d:/한의 의료서비스 통계/03_분석_및_수집_스크립트/jeonju_jeonbuk_full_simulation.json"
with open(output_path, "w", encoding="utf-8") as f:
    json.dump({
        'meta': {
            'jeonju': REGIONS['jeonju'],
            'jeonbuk': REGIONS['jeonbuk']
        },
        'disease_comparison': DISEASE_COMPARISON,
        'jeonju_scenarios': jeonju_results,
        'jeonbuk_scenarios': jeonbuk_results
    }, f, ensure_ascii=False, indent=2)

print(f"[완료] 전주시 및 전북특별자치도 시뮬레이션 완료 및 JSON 저장: {output_path}")

# ==============================================================================
# 시각화 차트 생성 (jeonju_jeonbuk_simulation_chart.png)
# ==============================================================================
fig, axes = plt.subplots(2, 2, figsize=(16, 12))

rates_x = [res['rate_pct'] for res in jeonju_results]

# 1) 전주시 vs 전북특별자치도 5개년 누적 순절감액 비교 (조 원 / 억 원)
jj_cum_sav = [res['cum_savings_mean'] for res in jeonju_results]
jb_cum_sav = [res['cum_savings_mean'] for res in jeonbuk_results]

axes[0, 0].bar(np.array(rates_x) - 1.5, jj_cum_sav, width=3, color='#3b82f6', alpha=0.85, label='전주시 5개년 누적 절감액 (억 원)')
axes[0, 0].bar(np.array(rates_x) + 1.5, jb_cum_sav, width=3, color='#10b981', alpha=0.85, label='전북특별자치도 5개년 누적 절감액 (억 원)')
axes[0, 0].set_title('① 참여율별 5개년 누적 순재정 절감액 (전주시 vs 전북특별자치도)', fontsize=12, fontweight='bold')
...
# 2) 5년차 노인 1인당 연평균 진료비 변화 (전주시 vs 전북전체)
jj_eld_y5 = [res['yearly'][4]['eld_mean'] / 10000.0 for res in jeonju_results]
jb_eld_y5 = [res['yearly'][4]['eld_mean'] / 10000.0 for res in jeonbuk_results]

axes[0, 1].plot(rates_x, [540.0]*10, 'b--', label='전주시 노인 Baseline (540.0만 원)', alpha=0.5)
axes[0, 1].plot(rates_x, [585.0]*10, 'g--', label='전북전체 노인 Baseline (585.0만 원)', alpha=0.5)
axes[0, 1].plot(rates_x, jj_eld_y5, 'o-', color='#1e88e5', linewidth=2.5, markersize=7, label='전주시 5년차 노인 1인당 진료비')
axes[0, 1].plot(rates_x, jb_eld_y5, 's-', color='#059669', linewidth=2.5, markersize=7, label='전북전체 5년차 노인 1인당 진료비')
...
# 3) 전북 14개 시군 광역 가용 방문진료 슬롯 및 전담 한의사 수
jb_docs = [res['km_doctors'] for res in jeonbuk_results]
jb_slots = [res['annual_visit_slots'] / 10000.0 for res in jeonbuk_results] # 만 회

axes[1, 0].bar(rates_x, jb_slots, color='#8b5cf6', alpha=0.8, width=6, label='전북 광역 연간 가용 방문슬롯 (만 회)')
ax_twin = axes[1, 0].twinx()
ax_twin.plot(rates_x, jb_docs, 'd-', color='#f59e0b', linewidth=2.5, markersize=7, label='참여 전담 한의사 수 (명)')
axes[1, 0].set_title('③ 전북특별자치도 참여율별 가용 방문진료 인프라 및 슬롯 공급량', fontsize=12, fontweight='bold')
...
axes[1, 1].bar(x_idx + w/2, km_c, width=w, color='#3b82f6', alpha=0.85, label='한의 방문진료·통합케어 (만 원)')

for i in range(len(d_labels)):
    pct = DISEASE_COMPARISON[i]['saving_pct']
    axes[1, 1].text(i, max(conv_c[i], km_c[i]) + 40, f"-{pct:.1f}%\n({savings_c[i]:,.0f}만↓)", 
                    ha='center', va='bottom', fontsize=9, fontweight='bold', color='#10b981')
...
plt.tight_layout()
chart_path = "d:/한의 의료서비스 통계/jeonju_jeonbuk_simulation_chart.png"
plt.savefig(chart_path, dpi=300)
plt.close()
print(f"[완료] 전주시 및 전북특별자치도 종합 차트 저장: {chart_path}")
